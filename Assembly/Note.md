...
![8259级联工作示意图](../res/img/Assembly/8259级联工作示意图.png)
...
- **普通全嵌套方式**
  ...
  - 最常用的一种工作方式。
- 特殊全嵌套方式
  - 在ISR的Di位置位期间，**禁止再发生低级优先权的中断**，但允许**同级和高级**优先权中断的嵌套
  - 级联情况下，主片采用特殊全嵌套方式，从片采用普通全嵌套方式。
...
- **普通中断结束方式**
  - **配合全嵌套优先权方式使用**
  - 当CPU用输出指令往8259A**发出普通中断结束EOI命令**时，8259A就会把所有正在服务的中断中优先权最高的ISR位复位
- 特殊中断结束方式
  - 配合循环优先权方式使用
  - CPU在程序中向8259A发送一条特殊中断结束命令，
    - 这个命令中指出了要清除哪个ISR位
...
- **普通屏蔽方式**
  - **将IMR的Di位置1，则对应的中断IRi被屏蔽，**
    - 该中断请求不能从8259A送到CPU
  - 如果IMR的Di位为0，则允许IRi中断产生
- 特殊屏蔽方式
  - 将IMR的Di位置1，对应的中断IRi被屏蔽的同时，使ISR的Di位清0。
  - 用于一种特殊要求的场合：在执行较高级的中断服务时，由于某种特殊原因，需要开放较低级别的中断请求。

---
#### （5）中断触发方式
- 边沿触发方式
  - 8259A将中断请求输入端出现的上升沿作为中断请求信号
  - ![边沿触发方式](../res/img/Assembly/边沿触发方式.png)
- 电平触发方式
  - 中断请求端出现的高电平是有效的中断请求信号
  - ![电平触发方式](../res/img/Assembly/电平触发方式.png)

---
#### （6）数据线连接方式
- 缓冲方式
  - 8259A的数据线需加缓冲器予以驱动
  - 8259A把SP*/EN*引脚作为输出端，输出允许信号，
    - 用以锁存或开启缓冲器
- 非缓冲方式
  - SP*/EN*引脚为输入端
  - 若8259A级连，由其确定是主片或从片 
...
---
### 7.3.5 8259A的初始化命令字和初始化编程
- 初始化编程
  - 8259A开始工作前，必须进行初始化编程，
    - 即给8259A写入**初始化命令字ICW**
- 中断操作编程
  - 在8259A工作期间，可以写入**操作命令字OCW**将选定的操作传送给8259A，使之按新的要求工作
  - 还可以读取8259A的信息，以便了解他的工作状态


---
### 7.3.6 8259A的操作命令字及应用
#### 8259A的控制字
- 8259A共有7个控制字
  - 4个初始化命令字
    - ICW1~ICW4
...
- 必须按照ICW1～ICW4顺序写入
- ICW1和ICW2是必须送的
- **是否写入ICW3和ICW4，由ICW1决定**。

----
###### ICW1的格式和各位的定义如下
![ICW1的格式和各位的定义](../res/img/Assembly/ICW1的格式和各位的定义.png)
...
###### ICW2的格式和含义
  ...
  - 写到8259A的**奇地址端口**
  - 其格式和各位定义如下:
    ![ICW2格式与含义](../res/img/Assembly/ICW2格式与含义.png)
    - 起作用的只是高5位
    - 低3位是对应的引脚编号

---
###### 8259A的中断向量
![8259A的中断向量](../res/img/Assembly/8259A的中断向量.png)

---
###### ICW3的格式和含义
  - **主片/从片的初始化命令字**
  - 写到8259A的**奇地址端口**
  - **只有在一个系统中使用了两片以上8259A并级联时，才需要写入ICW3** 
- **主片ICW3**：
  - **规定哪一个IR引脚接有从片**
  - $IR_i$
    - 接有从片：
    ...
    - 未接有从片：
      - 对应位设置为“0” 
  - 例如：主8259A的IR0、IR3上分别接有从片，
    - 则主片的ICW3=00001001。
    ![ICW3格式](../res/img/Assembly/ICW3格式.png)
- **从片ICW3**:  
  - **表明从片接在主片的哪一根IR线上**。
  ![从片ICW3格式](../res/img/Assembly/从片ICW3格式.png) 

---
###### ICW4的格式和含义
  - 方式控制初始化命令字 
  - 写入**奇地址端口**
  ![ICW4的格式和含义](res/ICW4的格式和含义.png)
---
###### **初始化举例**
- 某8086/8088系统采用**一片8259A**，要求工作在**全嵌套方式**下，**不用中断自动结束**方式，**不用缓冲**方式，中断请求为**边沿触发**方式，**中断类型码为0B0H~0B7H**。8259A的**端口地址假定为20H和21H**，试编写初始化程序。
  ```
  MOV  AL,13H   ；ICW1，边沿触发，单片，写ICW4
  OUT  20H,AL   
  MOV  AL,0B0H  ； ICW2，中断类型码高5位为10110
  OUT  21H,AL
  MOV  AL,01H    ； ICW4，非自动结束方式，全嵌套，非缓冲方式
  OUT  21H,AL

  ```

---
- 【例7-2】IBM PC/AT机中使用两片8259A管理中断，硬件连线如图 7‑13所示。
  - **主片**
    - IR2接有从片
    - 边沿触发，非缓冲方式
    - 起始中断类型号为08H(IR0)。
  ...
  ![初始化举例1](../res/img/Assembly/初始化举例1.png)
  - 主片8259A初始化
    ```
    MOV  AL,00010001B	;ICW1:边沿,ICW4,ICW3
    OUT  20H, AL	 	 ;写入ICW1(A0=0地址端口）
    MOV  AL, 00001000B	 ;ICW2:IR0中断类型号为08H
    OUT  21H, AL	  	 ;写入ICW2 (A0=1地址端口）
    MOV AL, 00000100B	 ;ICW3:主片IR2接有从片
    OUT  21H, AL	 	 ;写入ICW3 (A0=1地址端口）
    MOV  AL, 00010001B	 ;ICW4:非缓冲,正常中断结束				        ; 特殊全嵌套方式，主片
    OUT  21H, AL	  	 ;写入ICW4 (A0=1地址端口）
  ...
  - 从片8259A初始化
    ...
    MOV  AL,00010001B 	 ;ICW1:边沿,ICW4,ICW3
    OUT  0A0H, AL	  	 ;写入ICW1
    MOV  AL, 01110000B	 ;ICW2:IR0中断类型号为70H
    OUT  0A1H, AL	 	 ;写入ICW2	 
    MOV  AL, 00000010B	 ;ICW3:从片接主片的IR2
    OUT  0A1H, AL	 	 ;写入ICW3
    MOV  AL, 00000001B	 ;ICW4:非缓冲,正常结束, 				 	 ;非特殊全嵌套方式，从片
    OUT  0A1H, AL	 	 ;写入ICW4
...
- OCW共有3个：OCW1～OCW3
- 写入时**没有顺序要求**，需要哪个OCW就写入哪个OCW

---
###### $OCW_1$
- 中断屏蔽字
- **内容写入中断屏蔽寄存器IMR，各位互相独立**。
- 写入8259A的**奇地址端口**
- 其格式和各位定义如下:
  ![OCW1格式及定义](../res/img/Assembly/OCW1格式及定义.png)
- OCW1的应用
  - 某系统中有3个中断源，
    - 分别连接到8259A的IR0、IR2和IR3上。
  - 8259A的端口地址假定为80H、81H。则屏蔽字为
    ```
    MOV AL,11110010B  ;为0 的位允许中断,为1的位中断被屏蔽
    OUT 81H,AL
    ```
  - 若系统中再增加一个IR1上的中断源
    ```
    IN 	   AL,81H
    AND  AL,11111101B	;只将IMR中的D1位清“0”
    OUT  81H,AL
    ```
  - 当IR1的所有中断全部完成后 
    ```
    IN 	  AL,81H
    OR	  AL,00000010B	;只将IMR中的D1位置“1”，
    OUT  81H,AL
    ```
...
- 设置优先级循环方式和发中断结束命令
- 写入**偶地址端口**
![OCW2](../res/img/Assembly/OCW2.png)
- D5位EOI
  - EOI=1
    - 在非自动中断结束方式,当中断服务程序执行完时通过使EOI置1
      - 通知8259A将此次中断在ISR寄存器中相应的位置清0;
  - EOI=0
    - 在中断自动结束方式,不需要发送中断结束命令
- D4D3=00
  - 标志位
    - 也叫特征位
- $D_2$ ~ $D_0$位($L_2$ ~ $L_0$)
  - 指定中断等级
    - 000 ~ 111 对应0级 ~ 7级中断
---
![](../res/img/Assembly/OCW2-1.png)
- OCW2的应用
  - 当8259A设置成非自动结束方式时，**必须在中断服务程序结束之前，发中断结束命令**。
  - 8259A的端口地址假定为80H、81H。则普通中断结束命令为：
    ```
    MOV AL, 0010 0000B  ;20H，EOI位置1，发中断结束命令
    OUT 80H,AL
    ```
  - 如果8259A级联（例7-2系统），从片的中断服务程序结束之前必须发两个EOI命令，分别发给从片和主片。
    ```
    MOV AL,0010 0000B     ; 发普通中断结束命令
    OUT  0A0H, AL	    ;先向从片发中断结束命令
    OUT  20H, AL 	               ;后向主片发中断结束命令
...
- $OCW_3$必须写入**偶地址端口**
- $OCW_3格式与各位定义$
  ![OCW3格式及各位定义](../res/img/Assembly/OCW3格式及各位定义.png)
  - D6位(ESMM)
    - 特殊屏蔽模式使能位
      - ESMM = 1
        - SMM 位有效
...
- 查询有无中断请求
- 查询字的格式及其各位含义:
  ![查询字的格式及其各位含义](../res/img/Assembly/7.3.6-查询字的格式及其各位含义.png)

---
##### 2） 状态字IRR
- 其内容表明哪个输入引脚有中断请求
  ![IRR](../res/img/Assembly/7.3.6-IRR.png)

---
##### 3） 状态字ISR
- 其内容表明哪个中断正在被服务
  ![](../res/img/Assembly/7.3.6-ISR.png)

---
##### 4） 状态字IMR
- 其内容表明哪个中断请求被屏蔽
  ![IMR](../res/img/Assembly/7.3.6-IMR.png)
...
- 中断查询方式
  ```
  MOV  AL, 00001100B  ; 0CH，OCW3设置查询方式
  OUT  20H, AL        ; 写入A=0的偶地址端口
  IN   AL, 20H        ; 读中断查询字的内容到AL中
  ```
- 中断请求寄存器IRR的读取
  ```
  MOV  AL, 00001010B  ; 0AH，发读取IRR的OCW3命令
  OUT  20H, AL        ; 写入A=0的偶地址端口
  IN   AL, 20H        ; 读IRR内容到AL中
  ```
- 中断服务寄存器ISR的读取
  ```
  MOV AL, 00001011B   ; 0BH,发读取ISR的OCW3命令
...
---
## 7.4 中断应用举例
- 中断方式下，完整的输入输出程序应由二个程序模块配合完成：
  - 主程序
    - 完成输入输出所需要的“初始化”工作和结束处理工作；
  - 中断服务程序
    - 完成数据传输和输入输出控制工作
...
- （3）接口的初始化
  - 设置接口工作方式，设置中断开放位等。
- （4）中断服务程序的初始化：
  - 设置中断服务程序使用的缓冲区指针，计数器，状态位等。
- **注意**
  - 中断服务程序的指针、计数器、状态位等**只能存放在内存单元**
  - 对于输出过程，应在主程序中启动第一次输出。
  - 输入输出完成之后，主程序要根据需要做好结束工作。
    - 如：处理输入的数据，关闭中断等。

---
### 2)中断服务程序设计
- （1） 保护现场：
  - 把所有中断服务程序里使用、会改变值的寄存器压入堆栈。
- （2）开放中断：
  - 允许CPU响应优先级更高、更紧急的中断。
- （3）装载数据缓冲区指针、计数器。
- （4）输入输出处理：
- （5）关闭中断：
  - **关闭中断**可以避免不必要的中断嵌套，**发EOI命令**。
- （6）恢复现场：
...
---
#### 中断服务程序应注意的几个问题
- 中断服务程序要**短小精悍**，**运行时间短**，执行一次中断服务程序的时间要大大少于二次中断的时间间隔。对于耗费时间多的数据处理工作，应交由主程序完成。
- 一般的情况下，**应避免在中断服务程序内进行DOS功能调用**。那样做，可能产生这些程序的“重入”。DOS功能调用程序不具备重入功能，会产生难以预料的结果。需要进行控制台IO操作可以使用BIOS调用。
- 在输入输出处理完成后，**一定要向8259A发送中断结束命令（EOI）**。
  - **如果是级联的8259A的从片上的中断**
    - 则需要向主片和从片分别发送中断结束命令。
  - 否则，该设备的下一次中断就不能被响应，比它级别低的中断从此也不能被响应。


---
### 中断应用举例【例7-3】
假设某应用系统通过一组8位开关的状态控制8个发光二极管，某开关闭合对应的发光二极管点亮。要求8个开关的状态全部设置完成后，才能点亮对应的发光二极管，当开关的数据为0FFH时，程序结束。完成软硬件设计。
- 1、硬件电路设计
  ![硬件电路设计](../res/img/Assembly/7.4-硬件电路设计.png)
...
- 2、软件设计——主程序
  ...
  ;中断向量表初始化
    MOV AX, 0
    MOV ES, AX          ; 中断向量表段基地址为0000H
    MOV BX, 42H*4       ; 中断类型号为42H
    MOV AX, OFFSET  INTRPROC
    MOV ES:[BX], AX     ; 向量表中低地址存放偏移地址
    MOV AX, SEG INTRPROC
    MOV ES:[BX+2], AX   ; 向量表中高地址存放段基地址
  ;8259A初始化
    MOV AL, 00010011B   ; ICW1：边沿触发,单片系统,需要ICW4
    OUT 80H, AL
    MOV AL, 01000000B   ; ICW2：类型号从40H开始
    OUT 82H, AL
    MOV AL, 00000001B   ; ICW4：普通全嵌套,非缓冲,非自动结束
    OUT 82H, AL
    MOV AL, 11111011B   ; OCW1，开放IR2请求的中断屏蔽位
    OUT 82H, AL

  ;中断服务程序初始化
        MOV DONE, 0     ;完成标志清0
        STI
  WAIT: CMP DONE, 1     ;DONE=1表示所有中断完成
        JNZ WAIT
        MOV AX, 4C00H   ;返回DOS
        INT 21H
...
- 2、软件设计——中断服务子程序
    ...
    INTPROC PROC FAR
          ...
          MOV AX, SEG DONE
          MOV DS, AX
          IN  AL, 0A0H    ;从输入端口读入一个数据
          OUT 0C0H, AL    ;从输出端口输出,点亮发光二极管
          CMP AL, 0FFH    ; 判断读入的开关量是否为0FFH
          JNZ EXIT
          MOV DONE, 1     ;输入数据为0FFH,置结束标志
          IN  AL, 82H     ;设置IR2的中断屏蔽位
          OR  AL, 00000100B
          OUT 82H, AL
    EXIT: CLI             ;关中断,准备返回
          MOV AL, 20H     ;OCW2, 发EOI命令
          OUT 80H, AL
          POP DS          ;恢复现场
          POP AX
          IRET            ;中断返回
...
- 串行通信
  - 串行通信
    - 将数据分解成二进制位，用一条信号线**一位一位顺序传送**的方式
    - **要点**
      - 按位传输，同一时刻仅传送一位。
    - 特点：  
      - 通信线少、成本低，但通信速度慢
    - 用途：
      - 适用于远距离数据传输。也常用于速度要求不高的近距离数据传送
  - PC系列机上有两个串行异步通信接口。
  - 键盘、鼠标器与它们连接的接口间采用串行数据传送
    - 它们的接口与CPU之间仍是以并行方式传输数据。
  - 在网络传输中，使用**高级数据链路控制协议HDLC**（串行同步通信）
...
---
### 8.1.1 8255A的内部结构
![内部结构](../res/img/Assembly/8.1.1%208255A%20的内部结构.png)
- （1）三个数据端口A、B、C 
  - 三个8位的输入输出端口，
  - 可用指令将它们分别设置成输入或输出端口。
- （2）A组控制、B组控制 
...
![与处理器接口](../res/img/Assembly/8.1.1-与处理器接口.png)
...
---
##### 输入时序
![](../res/img/Assembly/8.1.3-方式0输入时序.png)
- **请体会这里8255A的数据缓冲作用**

---
##### 输出时序
![](../res/img/Assembly/8.1.3-方式0输出时序.png)
...
-----
##### 输入引脚：A端口
![](../res/img/Assembly/8.1.3-方式1输入引脚：A端口.png)
- **方式1需借用端口C用做联络信号**
  - **同时还具有中断请求和屏蔽功能**

---
##### 输入引脚：B端口
![](../res/img/Assembly/8.1.3-方式1输入引脚：B端口.png)
- INTE触发器对应端口C作应答联络信号的输入信号
  - 只要对相应位置位/复位就可以控制INTE触发器
- 选通输入方式下
  - 端口A的INTEA对应**PC4**;
  - 端口B的INTEB对应**PC2**.
...
- STB*
  - 选通信号，低电平有效
  - 由外设提供的输入信号
    - 当其有效时，将输入设备送来的数据锁存至8255A的输入锁存器
- IBF
  - 输入缓冲器满信号，高电平有效
  - 8255A输出的联络信号
    - 当其有效时，表示数据已锁存在输入锁存器
...
---
##### 输入时序
![](../res/img/Assembly/8.1.3-方式1输入时序.png)
- **STB*和IBF是外设和8255A间的一对应答联络信号**
  - **为的是可靠地输入数据**
...
- 对INTE的操作通过写入端口C的对应位实现
  - INTE触发器对应端口C的位是作应答联络信号的输入信号的那一位
    - 只要对那一位置位/复位就可以控制INTE触发器
- 选通输入方式下
  - 端口A的INTEA对应PC4
  - 端口B的INTEB对应PC2
...
---
##### 输出引脚：A端口
![](../res/img/Assembly/8.1.3-方式1输出引脚：A端口.png)

---
##### 输出引脚：B端口
![](../res/img/Assembly/8.1.3-方式1输出引脚：B端口.png)
- **端口A的INTEA对应PC6**
- **端口B的INTEB对应PC2**
...
- OBF*
  - 输出缓冲器满信号，低有效
  - 8255A输出给外设的一个控制信号，
    - 当其有效时，表示CPU已把数据输出给指定的端口，外设可以取走
- ACK*
  - 响应信号，低有效
...
- INTR
  - 中断请求信号，高有效
  - 当输出设备已接受数据后
    - 8255A输出此信号向CPU提出中断请求，要求CPU继续提供数据
> - 端口A的INTEA对应PC6
> - 端口B的INTEB对应PC2

---
##### 输出时序
![](../res/img/Assembly/8.1.3-方式1输出时序.png)
- $OBF*$ 和$ACK*$是外设和8255A间的一对应答联络信号，
  - 为的是可靠地输出数据

---
##### 联络信号
![](../res/img/Assembly/8.1.3-方式1的联络信号.png)
...
- 方式2的数据输出过程与方式1的输出方式有一点不同：
  - **数据输出时8255A不是在$OBF*$有效时向外设输出数据**
    - **而是在外设提供响应信号$ACK*$时才送出数据**

---
##### 双向引脚
![](../res/img/Assembly/8.1.3-方式2双向引脚.png)

---
##### 双向时序
![](../res/img/Assembly/8.1.3-方式2双向时序.png)
...
#### （1）写入方式控制字
- 控制字格式
![控制字格式](../res/img/Assembly/8.1.4写入方式控制字：控制字格式.png)
  - D3~6控制A组控制逻辑
    - 控制A口和C口的上半部
  - D0~2控制B组控制逻辑
    - 控制B口以及C口的下半部
  > - 注:A口B口的某些工作方式可能会征用C口的一些引脚
  > - 这时上图中的方式选择控制字对C口的控制只对没有被征用的引脚生效
...
- 方式控制字：
  - $10110001B$
    - | 1 | 01 | 1 | 0 | 0 | 0 | 1 | B |
      | - | - | - | - | - | - | - | - |
      |   | D6D5 | D4 | D3 | D2 | D1 | D0 |
      | 默认 | 方式1 | A口输入 |  C口上半输出 | 方式0 | 端口B输出 | 端口C下半输入 | 二进制 |
  - 或$B1H$
    - | B | 1 | H |
      | - | - | - |
      | 1011B | 0001B |
- 若8255A端口地址为60H，61H，62H，63H
- 初始化的程序段：
  ```
  mov dx,63h   ；控制端口为63H
  mov al,0b1h  ；方式控制字
...
#### （2）读写数据端口
- 初始化编程后：
  - 当数据端口作为输入接口时，执行输入**IN**指令将从输入设备得到外设数据
  - 当数据端口作为输出接口时，执行输出**OUT**指令将把CPU的数据送给输出设备
- 8255A具有**锁存输出数据**的能力
  - 对输出方式的端口同样可以输入
  - 不是读取外设数据
  - **读取的是上次CPU给外设的数据**
...
- 利用8255A的输出锁存能力，可实现**按位输出控制**
- 对输出端口B的PB7位置位的程序段：
```
mov dx,61h  ; B端口假设为61H
in al,dx    ; 读出B端口原输出内容
or al,80h   ; 使PB7＝1
out dx,al   ; 输出新的内容
```
...
#### （3）读写端口C：归纳1
- C端口的控制：分成两个4位端口分别控制
  - 只能以方式0工作，可分别设置成输入或输出
  - 上部（**PC4～PC7**）和A端口由A组控制，
  - 下部（**PC0～PC3**）和B端口由B组控制
  - 当A和B端口工作在方式1或方式2时，C端口的部分引脚将被征用。没有被征用的引脚可设定工作在方式0
...
- **通过端口C的I/O地址：**
  - 向C端口直接**写入字节数据**
  - 数据被写进C端口的输出锁存器，并从输出引脚输出，但**对设置为输入的引脚无效**
- 通过**控制端口**
  - **写入**端口C的**位控制字**，**使C端口的某个引脚输出1或0**

----
  
  #### 端口C的位控制字
- ![端口C的位控制字](../res/img/Assembly/8.1.4-端口C的位控制字.png)
  - D4~6没含义
    - 通常情况下设置为0
  - 最高位恒为0
...
---
##### 示例
8255A控制端口地址237H，现要对PC7置1，将PC6清0。
- 方法一：
  ```
  MOV  DX,237H
  MOV  AL,0FH
...
- 读取C端口的数据有两种情况
  - **未被A和B端口征用的引脚**：
    - 将从定义为输入的端口读到引脚输入信息；
    - 将从定义为输出的端口读到输出锁存器中的信息
  - **被A和B端口征用作为联络线的引脚**：
    - 将读到反映8255A状态的**状态字**

---
##### 端口C的状态字
![端口C的状态字](../res/img/Assembly/8.1.4-端口C的状态字.png)
- 这只是一张总结表,实际读取的值具体情况具体分析
- C口可以作为数据/状态/控制口使用
...
---
#### 例1 方式0 打印机
![用8255A方式0与打印机接口](../res/img/Assembly/8.1.5-用8255A方式0与打印机接口.png)
...
- BUSY：
  - 忙，表示打印机不能接收数据
- ACK*：
  - 响应，打印机接收一个数据字节后就回送一个响应的负脉冲信号
    - 脉宽约为$5μs$
- 8位数据的可靠输出通过选通STROBE*、响应ACK*和忙BUSY三个联络信号控制

---
##### 打印机接口的信号与时序
![打印机接口的信号与时序](../res/img/Assembly/8.1.5-打印机接口的信号与时序.png)

---
##### 8255A的初始化
```
mov dx,303h       ; 控制端口地址：303H
mov al,10000001b  ; 方式控制字：81H
out dx,al         ; A端口方式0输出，C端口上输出、下输入
mov al,00001111b  ; 端口C的复位置位控制字，使PC7＝1
out dx,al
```
...
##### 打印子程序:查询式输出
```
      printc proc   ; 入口参数AH=打印数据
      push ax
      push dx
prn:  mov dx,302h   ; 读取端口C
      in al,dx      ; 查询打印机状态
      test al,04h   ; PC2＝BUSY＝0？
      jnz prn   ; PC2＝1，打印机忙，则循环等待
                ; PC2＝0，打印机不忙，则输出数据
      mov dx,300h   ; 端口A地址：300H            
      mov al,ah
...
##### 打印子程序:打印
```
        mov dx,303h       ; 从PC7送出控制低脉冲
        mov al,00001110B  ; 置STROBE*＝0
        out dx,al
        nop               ; 产生一定宽度的低电平
        nop
        mov al,00001111B  ; 置STROBE* ＝1
        out dx,al         ; 最终，STROBE*产生低脉冲信号
        pop dx
        pop ax
        ret
printc  endp
...
---
#### 例8-5 方式一 打印机
8255A **工作在方式1**，将BUFF开始的缓冲区中以“$”为结束符的字符串从打印机输出打印。
![例8-5](../res/img/Assembly/8.1.5-例8-5.png)
- 根据译码电路可知
  - 8259A的地址为20H和22H；
  - 8255A地址为10H、12H、14H和16H
- 8255A方式1与打印机接口时序配合
![](../res/img/Assembly/8.1.5-例8-5-1.png)

##### 查询方式打印程序
```
      MOV SI,  OFFSET   BUFF
      MOV AL,0A0H   ;8255A方式字: 1010XXXXB。
      OUT 16H,AL
      MOV AL,0CH    ; 使INTEA（PC6）为0 ，禁止中断。
      OUT 16H,AL
WAIT: IN  AL,14H    ;读C口状态
      TEST AL,80H   ;检测PC7是否忙
      JZ WAIT       ;PC7=0,输出缓冲区满，则等待
      MOV AL,[SI]   ;PC7=1，取一个字符输出
      CMP AL,‘$’    ;是否结束符 
      JZ DONE       ;是结束符，程序结束
      OUT 10H,AL    ;不是结束符，从A口输出;自动产生STB#信号
      INC SI
      JMP WAIT      ;打印下一个字符
DONE: ……
...
MAIN: MOV  AL, 0A0H   ;8255A方式字
      OUT  16H,  AL    	
      MOV  AL,  0DH   ;PC6置1，INTE=1
      OUT  16H,  AL   ;使8255A A口输出允许中断
      ……
      ……              ;设置中断向量(略), 参考7.4节
```
...
###### 主程序
```
        MOV POINT,OFFSET  BUFF  ; 设置地址指针
        MOV DONE,0  ; 中断结束标志位,DONE=1所有字符打印完毕

        IN 	AL,22H  ; 开放8259A的中断屏蔽位，IMR3=0
        AND AL,11110111B
        OUT 22H,AL

        STI         ; 开中断
        INT 0BH     ; 调用中断服务程序打印第一个字符
NEXT:   CMP DONE,0  ; DONE=0,打印还没有结束，继续等待打印结束
        JZ  NEXT                  
INTEND: ……          ; 所有的中断完成，做中断结束处理
```
...
ROUTINTR  PROC
        ...
        MOV AX, SEG  BUFF
        MOV DS, AX
        MOV DI, POINT ; 取出将要打印的字符的地址,通过DI间接访问
        INC POINT   ; 地址指针加1，为下一次输出做准备

        MOV AL, [DI]  ;取出将要打印的字符
        CMP AL, '$'
        JZ FINISH

        OUT 10H, AL ;从A口输出一个字符,产生STB*通信号 

        JMP EXIT

FINISH: MOV DONE, 1 ; 所有字符都打印完成DONE=1，与主程序同步
        MOV AL, 0CH
        OUT 16H, AL ; 禁止8255A A口输出中断

        IN  AL, 22H ; 设置8259A的中断屏蔽位，IMR3=1
        OR  AL, 00001000B
        OUT 22H, AL

EXIT:   CLI         ; 关中断,准备返回
        MOV AL, 20H ; 发8259A的EOI命令，从8259A的偶地址输出
        OUT 20H, AL
...
---
## 8.4 可编程定时/计数接口芯片8254
- 定时器和计数器
  - 定时控制在微机系统中具有极为重要的作用，计数是许多过程控制领域常用的功能
  - 它们都是由数字电路中的计数电路构成
    - **定时器**由数字电路中的计数电路构成，通过记录高精度晶振脉冲信号的个数，**输出准确的时间间隔**
    - 计数电路如果记录外设提供的具有一定随机性的脉冲信号时，它主要**反映脉冲的个数**（进而获知外设的某种状态），常又称为**计数器**

---
- 定时功能的实现方法
  - **软件延时**
    - 利用微处理器执行一个延时程序段实现
    - 不用硬件，但**占用CPU时间、定时精度不高**，随系统时钟频率改变
  - **不可编程的硬件定时**
    - 采用**分频器、单稳电路**或简易定时电路控制定时时间
    - 定时电路简单、定时时间可以在一定范围改变
  - **可编程的硬件定时**
    - 软硬件相结合、用可编程定时器芯片构成一个方便灵活的定时电路
    - 具有多种工作方式、能够输出多种控制信号
...
- 8253/8254定时计数器
  ...
  - 每个计数器通道有6种工作方式
  - 可以按二进制或十进制（BCD码）计数
  - **8253计数频率为0~2.6MHz，8254为0~12MHz**
  - 计时时间长短可用软件设置，可由软件或硬件控制开始计数或停止计数
  - **8254可同时锁存1～3个计数器的计数值和状态值，供CPU读取** 
    - **8253每次只能锁存和读取一个通道的计数值**
  - **8254是8253的改进型**
![](../res/img/Assembly/8.4.1-8253.png)

---
- 8253/8254的内部结构和引脚
  ![](../res/img/Assembly/8253,8254的内部结构和引脚.png)
  > 计数器也称为"通道"/"计数通道"

---
#### （1）计数器
![](../res/img/Assembly/8.4.1-计数器.png)
- 计数初值存于**初值寄存器**；
- 在计数过程中，
  - **减1计数器**的值不断递减，
    - 而初值寄存器中的预置不变。
...
- 计数器的3个引脚
  ...
  - **OUT**计数器输出信号
    - 当一次计数过程结束（计数值减为0），OUT引脚上将产生一个输出信号，输出信号的形式由计数器的工作方式确定。
...
- $RD*$读信号
- $WR*$写信号
![](../res/img/Assembly/8.4.1-8254与处理器接口.png)
...
---
#### 方式0：计数器方式（计数结束中断方式）
![方式0](../res/img/Assembly/8.4.3-方式0.png)
- 信号注释
  - **CLK**时钟输入信号
  - **GATE**门控输入信号
    - 一直为高电位才能计数
    - 若其意外变成低电位则计数器会暂停计数
      - 只有当$GATE$门控输入信号恢复成高电位时才会**继续计数**
        - 区别于方式4当$GATE$信号变成低电位之后再次变成高电位时才会**重新计数**
  - **OUT**计数器输出信号
    - 有一个由低到高的跳变
      - 这个跳变边沿可以作为中断请求信号
  - $WR*$写信号
...
---
#### 方式1:可重触发的单稳态触发器
![方式1](../res/img/Assembly/8.4.3-方式1：可重触发的单稳态触发器.png)
- 信号注释
  - **CLK**时钟输入信号
  - **GATE**门控输入信号
    - 要有一个跳变才能够开始计数
...
---
#### 方式2：频率发生器（分频器）
![](../res/img/Assembly/8.4.3-方式2：频率发生器（分频器）.png)
- 信号注释
  - **CLK**时钟输入信号
  - **GATE**门控输入信号
  - **OUT**计数器输出信号
...
---
#### 方式3：方波发生器
![](../res/img/Assembly/8.4.3-方式3：方波发生器.png)
- 信号注释
  - **CLK**时钟输入信号
  - **GATE**门控输入信号
  - **OUT**计数器输出信号
    - 当计数初值为偶数时
      - 在计数周期内,一般时间为高电位,一半时间为低电位
    - 当计数初值为奇数时
      - 高电位输出时间比低电位输出时间多一段
  - $WR*$写信号
- 特点
...
---
#### 方式4：软件触发选通信号
![](../res/img/Assembly/8.4.3-方式4：软件触发选通信号.png)
- 信号注释
  - **CLK**时钟输入信号
  - **GATE**门控输入信号  
    - $GATE$为低时终止计数
      - 当$GATE$再次变成高时**重新**从初值开始计数
        - 区别于方式0:
          - 若$GATE$意外变成低电位则计数器会暂停计数,只有当$GATE$门控输入信号恢复成高电位时才会**继续计数**
  - **OUT**计数器输出信号
    - 当计数初值为偶数时
      - 在计数周期内,一般时间为高电位,一半时间为低电位
    - 当计数初值为奇数时
...
---
#### 方式5：硬件触发选通信号
![](../res/img/Assembly/8.4.3-方式5：硬件触发选通信号.png)
...
---
### 8.4.4 8254的控制字与编程
- 8253/8254加电后的工作方式不确定
- 8253/8254必须初始化编程，才能正常工作
  - 写入控制字
  - 写入计数初值

![](../res/img/Assembly/8.4.4-8254控制字.png)

---
#### （1） 写入方式控制字
![](../res/img/Assembly/8.4.4-（1）%20写入方式控制字.png)
- 计数器
  - 00  计数器0
  - 01  计数器1
  - 10  计数器2
...
- 选择十进制（BCD码）
  ...
  - 0000代表最大值10000
> **计数值写入计数器各自的I/O地址**
> ![](../res/img/Assembly/8.4.4-8253,8254的IO地址.png)
...
- 计数在不断进行，应该将当前计数值先行锁存，然后读取：
  - 向控制字I/O地址：写入锁存命令
  - 从计数器I/O地址：读取锁存的计数值
> 读取计数值，要注意**读写格式**和**计数数制**

---
#### （4） 8254的读出控制字
![](../res/img/Assembly/8.4.4-(4)8254的读出控制字.png)
- D7D6和D0为标志位，必须为110。 
- D5=0锁存当前的计数值；
- D4=0锁存当前状态；
- D3~D1 
...
---
##### 读出控制字举例
【例8-16】设8254的端口地址为：0E0H~0E3H，将计数器2的16位计数器读出并保存在CX寄存器中，编程如下：

```
MOV AL, 0D8H  ; 读出控制字为11011000B
OUT 0E3H, AL  ; 锁存计数器2的计数值
IN  AL, 0E2H  ; 先读低8位
MOV CL, AL    ; 存低八位
IN  AL, 0E2H  ; 后读高8位
MOV CH, AL    ; 存高八位，CX中为读取到的结果

```

---
#### （5） 8254的状态字
![](../res/img/Assembly/8.4.4-(5)8254的状态字.png)
- D5~D0，与方式控制字设定相同。 
- D6=0，表示已装入初值，读取的计数值有效，
  - D6=1，表示计数值无效。
- D7=0表明OUT引脚当前输出低电平，
  - 9000D7=1表明OUT引脚当前输出高电平。

---
##### 例题
- 现有一个高精密晶体振荡电路，输入信号是脉冲波，频率为1MHz。
  - 要求利用8253做一个秒信号发生器，其输出接一发光二极管，以0.5秒点亮，0.5秒熄灭的方式闪烁指示。假定8253的通道地址为40H ~ 43H. 
  - （1）8253作一个分频电路，而且其输出应该是方波 
  - （2）分频系数N ：
    - $N = \frac{1 MHz}{1 Hz}  = \frac{1000000 Hz}{1 Hz}  = 1000000 > 2^{16}$
    - $N = 1000000 = 1000 \times 1000 = N_1 \times N_2$
  > 通道计数器级联实现分频系数超过$2^{16}$的分频要求
...
- 模拟量
  - 连续变化的物理量
  ![](../res/img/Assembly/10.1-模拟量.png)
- 数字量
  - 时间和数值上都离散的量

---
### 1.模拟输入输出系统
![](../res/img/Assembly/10.1-模拟输入输出系统.png)
- 传感器
  - 将各种现场的物理量测量出来并转换成电信号（模拟电压或电流） 
- 放大器
  - 把传感器输出的信号放大到ADC所需的量程范围
- 低通滤波器
  - 用于降低噪声、滤去高频干扰，以增加信噪比
- 多路开关
  - 把多个现场信号分时地接通到A/D转换器
- 采样保持器
  - 周期性地采样连续信号，
...
## 10.2 数-模(D-A)转换
### D/A转换器
![](../res/img/Assembly/10.1-DA转换器.png)

---
#### 运算放大器(1)
![](../res/img/Assembly/10.1-运算放大器(1).png)
- 1、开环放大倍数非常高，所需输入电压小
- 2、输入阻抗非常大，输入电流很小
- 3、输出阻抗非常小，驱动能力很强。
...
---
#### 运算放大器(2)
![](../res/img/Assembly/10.1-运算放大器(2).png)
$Ii +Io=0 \\ Vi/Ri =－Vo/Ro \\ Vo=－Vi*Ro/Ri$

---
### 10.2.1 数-模转换的原理
![](../res/img/Assembly/10.1-DA转换的基本原理.png)
![](../res/img/Assembly/10.1-DA转换器的原理图（1）.png)
![](../res/img/Assembly/10.1-DA转换器的原理图（2）.png)
![](../res/img/Assembly/10.1-DA转换器的原理图（3）.png)
![](../res/img/Assembly/10.1-DA转换器的原理图（4）.png)

---
#### 分辨率
- 指 **D/A 转换器所能产生的最小模拟增量**
- 是数字量最低有效位LSB所对应的模拟值△。
- 常用相对值（百分值）表示
  - $分辨率 =  △ / [(2^n-1 ) × △] = 1/ (2^n-1 )$
- 也可直接用D/A 转换器的位数表示。
...
---
### 10.2.2 DAC0832 芯片及其应用
- DAC0832是典型的**8位电流输出型**通用DAC芯片
![](../res/img/Assembly/10.2.2-DAC0832芯片.png)

---
#### 内部结构
![](../res/img/Assembly/10.2.2-DAC0832的内部结构.png)
...
- 两级缓冲寄存器
  - LE＝1，直通（输出等于输入）
  - LE＝0，锁存（输出保持不变）
![](../res/img/Assembly/10.2.2-DAC0832的工作方式.png)

---
##### 直通方式
- LE1＝LE2＝1
- 输入的数字数据直接进入D/A转换器
![](../res/img/Assembly/10.2.2-DAC0832的工作方式：直通方式.png)
...
- 两个寄存器之一始终处于直通状态
- 另一个寄存器处于受控状态（缓冲状态）
![](../res/img/Assembly/10.2.2-DAC0832的工作方式：单缓冲方式.png)

---
##### 双缓冲方式
- 两个寄存器都处于受控（缓冲）状态
- 能够对一个数据进行D/A转换的同时；输入另一个数据
![](../res/img/Assembly/10.2.2-DAC0832的工作方式：双缓冲方式.png)

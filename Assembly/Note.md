...
- 边沿触发方式
  ...
  - ![边沿触发方式](res/边沿触发方式.png)
- 电平触发方式
  - 中断请求端出现的高电平是有效的中断请求信号
  - ![电平触发方式](res/电平触发方式.png)
...
----
###### ICW1的格式和各位的定义如下
![ICW1的格式和各位的定义](res/ICW1的格式和各位的定义.png)
...
###### ICW2的格式和含义
  ...
  - 写到8259A的**奇地址端口**
  - 其格式和各位定义如下:
    ![ICW2格式与含义](./res/ICW2格式与含义.png)
    - 起作用的只是高5位
    - 低3位是对应的引脚编号

---
###### 8259A的中断向量
![8259A的中断向量](res/8259A的中断向量.png)
...
- **主片ICW3**：
  ...
  - 例如：主8259A的IR0、IR3上分别接有从片，
    - 则主片的ICW3=00001001。
    ![ICW3格式](./res/ICW3格式.png)
- **从片ICW3**:  
  - **表明从片接在主片的哪一根IR线上**。
  ![从片ICW3格式](./res/从片ICW3格式.png) 
...
- $OCW_3$必须写入**偶地址端口**
- $OCW_3格式与各位定义$
  ![OCW3格式及各位定义](res/OCW3格式及各位定义.png)
  - D6位(ESMM)
    - 特殊屏蔽模式使能位
      - ESMM = 1
        - SMM 位有效
...
- 查询有无中断请求
- 查询字的格式及其各位含义:
  ![查询字的格式及其各位含义](./res/7.3.6-查询字的格式及其各位含义.png)

---
##### 2） 状态字IRR
- 其内容表明哪个输入引脚有中断请求
  ![IRR](./res/7.3.6-IRR.png)
